...
Function Buscar(Var C: Cola; Elemento: TipoElemento; Var Posicion: String): Boolean;
var
  X: TipoElemento;
  CAux: Cola;
  i: Integer;
begin
  ...
  while (not C.EsVacia) do begin
    X:= C.Recuperar;
    if (Elemento.Clave = X.Clave) then begin
      Buscar:= True;
      Posicion:= Posicion + ' : ' + IntToStr(i);
    end;
    Inc(i);
    CAux.Encolar(X);
    C.DesEncolar;
...
Procedure Simplificacion(L:lista; Repetido: TipoElemento; i:integer; Posicion: PosicionLista);
var
  X: TipoElemento;
begin

  X:= L.Recuperar(Posicion);
  X.Valor1:= X.Valor1 + ' : ' + IntToStr(i);
  L.Actualizar(X, Posicion);
...
begin
  ...
  while (not P.EsVacia) do begin
    X1:= P.Recuperar;
    S:= 'C';
    if (Buscar(C, X1, S)) then begin
      X2.Clave:= X1.Clave;
      S:= S + ' : / P: ' + IntToStr(i);
      X2.Valor1:= S;
      if (L.Buscar(X2) = NULO) then begin
        L.Agregar(X2);
      end else
        Simplificacion(L, X1,i, L.Buscar(X1));
    end;
    Inc(i);
    PAux.Apilar(X1);
    P.DesApilar;
