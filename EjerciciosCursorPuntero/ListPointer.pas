...
Begin
  ...
  If Not EsVacia Then Begin
    I := 1;
    Q := Inicio;
  While (I < PLogica) And (Q <> Nulo) Do Begin Inc(I);
    Q := Q^.Prox;
  End;
...
begin
  ...
  if (not esVacia) then begin
    Q:= Inicio;
    while (Q <> NULO) do begin
      if (Q^.Datos.Clave = X.Clave) then begin
        Inc(Ocurrencias);
        Eliminar(Q);
      end;
      Q:= Q^.Prox;
...
begin
  ...
  if (not esLlena) then begin
    if (esVacia) then begin //Lista vacia agregar normalmente
      Agregar(X);
      QR:= Inicio;
    end else begin
      if (X.Clave < Inicio^.Datos.Clave) then begin //Elemento menor al primero de la lista
        Insertar(X, Inicio);
        QR:= Inicio;
      end else begin
        if (X.Clave >= Final^.Datos.Clave) then begin //Elemento mayor o igual al ultimo
          Agregar(X);
          QR:= Final;
        end else begin //Ninguno de los anteriores
          Q:= Inicio;
          Encontrado:= False;
          while (Q <> NULO) and (not Encontrado) do begin
            if (X.Clave < Q^.Datos.Clave) then begin
              Insertar(X, Q);
              QR:= Q;
              Encontrado:= True;
            end else
