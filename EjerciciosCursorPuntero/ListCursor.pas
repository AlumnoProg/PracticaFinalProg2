...
Interface
  ...
  Lista = Object
    ...
    Public
      ...
      Function EliminarOcurrencias(X: TipoElemento): LongInt;
      Function InsertarOrdenado(X:TipoElemento): PosicionLista;
    End;
...
Begin
  ...
  Else Begin
    If ValidarPosicion(P) Then Begin Q := Libre; // Tomo el primer libre disponible
      Libre := Cursor[Libre].Prox; // Encadeno el resto de los libres
      Cursor[Q].Datos := X;
      Cursor[Q].Prox := P;
...
Function Lista.EliminarOcurrencias(X: TipoElemento): LongInt;
var
  Ocurrencias: LongInt;
  PL: PosicionLista;
begin
  ...
  while (PL <> NULO) do begin
    if (Cursor[PL].Datos.Clave = X.Clave) then begin
      Inc(Ocurrencias);
      Eliminar(PL);
    end;
    PL:= Cursor[PL].Prox;
  end;
  EliminarOcurrencias:= Ocurrencias;
end;

function Lista.InsertarOrdenado(X:TipoElemento): PosicionLista;
var
  PL, PLR: PosicionLista;
  Encontrado: Boolean;
begin
  PLR:= NULO;
  if (not esLlena) then begin
    if (esVacia) then begin //Si la lista es vacia solo se agrega
      Agregar(X);
      PLR:= Inicio;
    end else begin
      if (X.Clave < Cursor[Inicio].Datos.Clave) then begin //Si X es menor al primer elemento
        Insertar(X, Inicio);
        PLR:= Inicio;
      end else begin
        if (X.Clave >= Cursor[Final].Datos.Clave) then begin //Si X es mayor o igual al ultimo elemento de la lista
          Agregar(X);
          PLR:= Final;
        end else begin //Si no es ninguno de los anteriores recorrer la lista
          PL:= Inicio;
          Encontrado:= False;
          while (PL <> NULO) and (not Encontrado) do begin
            if (X.Clave < Cursor[PL].Datos.Clave) then begin //Recorro hasta que se encuentre un elemento mayor a X
              Insertar(X, PL);
              PLR:= PL;
              Encontrado:= True;
            end else //Si no se pasa a la siguiente posicion de la lista
              PL:= Cursor[PL].Prox;
          end;
        end;
      end;
