...
Begin
  ...
  if (alSize >= Min) And (alSize <= Max) then Begin
    ...
    raiz := Nulo;
    q_items := 0;
    TDatoDeLaClave := avTipoClave; Size := alSize;
    Crear := OK;
...
Begin
  ...
  While Not(P.EsVacia) OR Not(RamaNula(Q)) Do Begin // Ciclo del Loop de llamada por derecha
    ...
    // Corto las llamada por izquierda. Tengo que desapilar e ir por derecha S := S + '.';
    X := P.Recuperar();
    Q := X.Valor2;
    P.DesApilar;
    Q := Q^.HD;
 ...
 // Proceso que resuelve la altura. “C” cuenta los pasos desde la raiz a cada nodo
  ...
  Begin
    ...
    If RamaNula(P) Then Begin
      ...
      // cada vez que llega a la hoja pregunta si la cantidad de pasos fue mayor que la de la hoja anterior
...
Begin
  P^.HD := Q; If Q <> Nulo Then Q_Items := Q_Items + ContarNodos(Q);;
End;
End.
