...
Var
  Altura: Integer;
  Procedure VerAltura(PA: PosicionArbol; AlturaR: Integer);
  begin
    if (not A.RamaNula(PA)) then begin
      if (PA = ABuscar) then begin
        If(AlturaR>Altura)Then
          Altura:= AlturaR;
      end else begin
        VerAltura(PA^.HI, AlturaR+1);
        VerAltura(PA^.HD, AlturaR+1);
      end;
    end;
  end;
begin
...
Function NodosAlMismoNivel(ABuscar: PosicionArbol): String;
var
  SR: String;
  Nivel: Integer;
  Procedure BuscarNodos(PA: PosicionArbol);
  begin

    if (not A.RamaNula(PA)) then begin
      BuscarNodos(PA^.HI);
      if(A.Nivel(PA)=Nivel)and(A.Recuperar(PA).Clave<>A.Recuperar(ABuscar).Clave)then begin
        SR:= SR + VarToStr(PA^.Datos.Clave) + ' , ';
      end;
      BuscarNodos(PA^.HD);
    end;
...
begin
  ...
  SR:= '';
  Nivel:= A.Nivel(ABuscar);
  BuscarNodos(A.Root);
  NodosAlMismoNivel:= SR;
