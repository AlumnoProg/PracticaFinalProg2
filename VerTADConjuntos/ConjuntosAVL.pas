...
Type
  ...
  Conjunto = Object
    ...
    Public
      ...
      Procedure Sort();
      Function BuscarElementos(X: TipoElemento): Boolean;
      Function SumarContenido(): LongInt;
    End;
...
Function conjunto.LlenarClavesRandom(alSize: LongInt; RangoDesde, RangoHasta: LongInt): Errores;
Var X: TipoElemento;
...
Var CU: Conjunto;
  ...
  Begin
    ...
    If Not items.RamaNula(P) Then Begin
      ...
      IO(items.hijoizquierdo(P)); X := items.recuperar(P);
      CU.Agregar(X);
      IO(items.hijoderecho(P));
...
Function Conjunto.BuscarElementos(X: TipoElemento): Boolean;
var
  Encontrado: Boolean;
  Procedure Buscar(Q: PosicionArbol; Var Flag: Boolean);
  var
    X2: TipoElemento;
  begin
    if (not items.RamaNula(Q)) and (not Flag) then begin
      Buscar(Q^.HI, Flag);
      X2:= items.Recuperar(Q);
      if (X.Clave = X2.Clave) then
        Flag:= True;
      Buscar(Q^.HD, Flag);
    end;
  end;
begin
  Encontrado:= False;
  Buscar(items.Root, Encontrado);
  BuscarElementos:= Encontrado;
end;

Function Conjunto.SumarContenido(): LongInt;
var
  Suma: LongInt;
  Procedure Recorrer(Q: PosicionArbol; Var Sumatoria: LongInt);
  var
    Elemento: TipoElemento;
  begin
    if (not items.RamaNula(Q)) then begin
      Recorrer(Q^.HI, Sumatoria);
      Elemento:= items.Recuperar(Q);
      Sumatoria:= Sumatoria + Elemento.Clave;
      Recorrer(Q^.HD, Sumatoria);
    end;
  end;
begin
  Suma:= 0;
  Recorrer(items.Root, Suma);
  SumarContenido:= Suma;
end;
